...
#    text_file.write('"{}" "{}" "{}" "{}" "{}" "{}"\n'.format(temp[i]["first_name"],
#        temp[i]["last_name"], temp[i]["_id"], temp[i]["address"]["state"], temp[i]["address"]["city"], temp[i]["address"]["zip"]))

text_file = open("merchants.txt", "w")
for i in range(0, len(temp)):
    #line = '' + str(temp[i]["first_name"]) + ' ' + str(temp[i]["last_name"]) + ' ' + str(temp[i]["_id"]) + ' ' + str(temp[i]["state"]) + ' ' + str(temp[i]["zip"])
    text_file.write('"{}" "{}" "{}" "{}" "{}" "{}"\n'.format(temp[i]["name"], temp[i]["_id"], temp[i]["address"]["city"],
	temp[i]["address"]["zip"], temp[i]["geocode"]["lat"], temp[i]["geocode"]["lng"]))
